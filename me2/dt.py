...
class DecisionTree:
    ...
    def record_outcome(self, team: int):
        ...
        # The bit-width of the encoded traversal is variable. (min, max) = (48, 69)
        ...
        leader1: Optional[bool] = self.checkpoints.get(
            Checkpoint.LEADER1, None)
        ...
        encoder.encode_ally_value_as_index(
            self.checkpoints[Checkpoint.LEADER2])
        encoder.encode_ally_value_as_index(escort)
        walk_unpick = self.checkpoints.get(
            Checkpoint.WALK_UNPICK, None) is not None
    ...
    def _choose_morinth(self, team: int):
        ...
        if Ally.Samara.value & team & self.loyal:
            ...
            self._choose_armor_upgrade(
                team | Ally.Morinth.value & ~Ally.Samara.value)
